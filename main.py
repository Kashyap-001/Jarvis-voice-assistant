    ...
    for f in os.listdir(TEMP_AUDIO_DIR):
        ...
        if os.path.isfile(file_path) and file_path.endswith(".mp3"):
            try:
                ...
                print(f"Cleanup error for file {file_path}: {e}")
...
def speak(text):
    ...
    unique_filename = os.path.join(TEMP_AUDIO_DIR, f"temp_{uuid.uuid4().hex}.mp3")
    ...
    while pygame.mixer.music.get_busy():
        time.sleep(0.1)
    time.sleep(0.3)  # Short pause to avoid overlap; do NOT delete file here
...
def processCommand(command):
    ...
    elif "open news" in lower_c and NEWSAPI_KEY:
        try:
